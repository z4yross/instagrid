...
export default function MobileActionOverlay({ visible, fabPosition }: Props) {
  ...
  const overlayX = positionLeft ? fabPosition.x + 60 + 16 : fabPosition.x - overlayWidth - 16
...
function ActionButton({
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => !disabled && (e.currentTarget.style.background = 'rgba(0, 0, 0, 0.6)')}
      onMouseLeave={(e) => !disabled && (e.currentTarget.style.background = 'rgba(0, 0, 0, 0.4)')}
    >
      {children}
    </button>
  )
